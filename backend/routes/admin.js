...
router.get('/cleaner/:cleanerId/clock-history', authMiddleware(['admin']), async (req, res) => {
  try {
    const admin = await User.findById(req.user.id);
    if (!admin || admin.role !== 'admin') {
      return res.status(403).json({ message: 'Access denied' });
    ...
    if (!cleaner) {
      return res.status(404).json({ message: 'Cleaner not found or not in your company' });
    }

    // Fetch clock history for this cleaner
    const clockHistory = await Clock.find({ userId: cleaner._id }).sort({ date: -1 });
